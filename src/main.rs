...
use rustml::environment::{line_world, grid_world, playable_monte_hall, monteHall, two_round_rock_paper_scissors, secret_env0dp::SecretEnv0Dp, secret_env1dp::SecretEnv1Dp, secret_env2dp::SecretEnv2Dp, secret_env3dp::SecretEnv3Dp, monty_hall};
...
use rustml::environment::monteHall::MonteHall;
    ...
    */
    ...
    fn process(environment: &str, algorithm: &str, save: bool, load: bool) {
        ...
        let mut algo = match algorithm {
            ...
            "dyna_q" => { Algo::DynaQ(
                DynaQModel::new(1000,  0.9,  0.9,  0.1, 10)
            )},
            "dyna_q+" => { Algo::DynaQPlus(
                DynaQPlusModel::new(1000, 0.95, 0.1, 0.1, 10, 0.001)
            )},
    ...
    process("secretenv0",  "dyna_q", true, false);
}